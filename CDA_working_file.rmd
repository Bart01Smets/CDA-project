...
```
To investigate the impact of these missing values, the distribution of the other variables is compared in the missing data vs. the non-missing data. This is done using the histograms.
...
# MSE
sse_simple <- sum(fit$residuals^2) #controle BIC: 1595*log(sse_simple/1595) + 2*log(1595)
...
We check assumptions linearity, independence of errors, homoscedasticity, and normality of errors. It is clear these assumptions are violated. In the next step, we extend the model by adding relevant predictors and reasses the assumptions.
...
data <- train_data[, c("logViolent", predictors), with = FALSE]

# Test reg with max 5 predictors
all_combos <- regsubsets(logViolent ~ ., data = data, 
                         nvmax = 5, nbest = 10, method = "exhaustive")
all_combos_sum <- summary(all_combos)
```

```{r BIC model selection2}
...
formula_multi <- as.formula(paste("logViolent ~", 
                                   paste(best_pred_sel, collapse = " + ")))
...
# Predictions on test set
pred_log <- predict(fit_log, newdata = test_data)
...
# MSE training
mse_final <- mean((exp(fitted(fit_log)) - 1 - train_data$ViolentCrimesPerPop)^2)
...
ss_tot <- sum((test_data$ViolentCrimesPerPop - mean(test_data$ViolentCrimesPerPop))^2)
r2_test <- 1 - ss_res/ss_tot
cat("\nR² on test set (original scale):", round(r2_test, 4), "\n")

#back to original scale 
org_scale <- exp(fitted(fit_log)) -1
ss_res_train <- sum((train_data$ViolentCrimesPerPop - org_scale)^2)
ss_tot_train <- sum((train_data$ViolentCrimesPerPop - mean(train_data$ViolentCrimesPerPop))^2)
r2_train_orig <- 1 - ss_res_train / ss_tot_train

cat("R² on training set (original scale):", round(r2_train_orig, 4), "\n") 
```
