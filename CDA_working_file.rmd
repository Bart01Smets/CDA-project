...
```
Datapoints in these plots are either outliers, and/or have high leverage in the model. If datapoints returned to be both outliers and have a high leverage, they can be seen as highly influencial in the model and most problematic, to check wether these are  observations errors, rare cases, or valid but extreme cases. They were extracted and manually checked. 
...
```
```{r}
# Extract all flagged observations
...
df1
df2 <- data.frame(
  R_squared = c(summary(fit_final)$r.squared, summary(fit_final_test)$r.squared),
  Adjusted_R_squared = c(summary(fit_final)$adj.r.squared, summary(fit_final_test)$r.squared),
  BIC_model = c(BIC(fit_final), BIC(fit_final_test))
)
rownames(df2) <- c("fit on training data","fit on test data")
df2
```
...
cat("Ratio MSPR/MSE:", round(mspr_test/mse_train_log, 4), "\n")
