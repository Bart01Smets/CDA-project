...
```{r}
library(olsrr)
diagplots <- olsrr::ols_plot_diagnostics(fit_log, print_plot = TRUE)
```
...
robust <- rlm(formula(fit_log), data = train_data, psi = psi.bisquare)
...
comparison <- data.frame(
  OLS = coef(fit_log),
  Robust = coef(robust),
  Difference = coef(fit_log) - coef(robust)
)
