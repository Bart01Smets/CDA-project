...
In his seminal work, Becker (1968) stated that the decision to commit crime is a rational choice where people weigh the benefits and costs against each other. It could then be argued that the incentive to commit crime is higher for people who have a lower income, as the benefits are larger for this group. Following this, we would then also expect that communities with a higher poverty rate will be associated with higher crime rates. Depending on the results of our analysis, these results could be used to inform relevant policies. It would, for example, give another argument for the implementation of redistributive policies: if an effect is found, policymakers should take this reduction in violent crime into account, next to an economic benefit. Our analysis hopes to shed further light on this issue.
...
train_data$log_racepctblack <- log(train_data$racepctblack + 1)
# Convert percent to proportion
p <- train_data$racepctblack / 100
...
# Logit transform
train_data$race_black_logit <- log(p / (1 - p))
train_data$race_black_asin <- asin(sqrt(train_data$racepctblack))



# 2. Fit your model using the NEW column
fit_log <- lm(logViolent ~ 
                PctPopUnderPov + 
                PctBSorMore + 
                race_black_logit + 
                PctImmig +
                PctPopUnderPov:PctImmig,
              data = train_data)
...
# 4. Build a clean predictor list WITH ONLY REAL VARIABLES
predictor_list <- rownames(multi_var_summary$coefficients)[-1]

main_terms <- predictor_list[!grepl(":", predictor_list)]
...
```{r}
library(olsrr)
diagplots <- olsrr::ols_plot_diagnostics(fit_final, print_plot = FALSE)
```
