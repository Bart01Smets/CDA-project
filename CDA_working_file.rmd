...
lm_cost_load <- lm(cost ~ load, data = airdata)
temp <- airdata
temp$resid_costload <- resid(lm_cost_load)
lm_utl_load <- lm(utl ~ load, data = temp)
temp$resid_utlload <- resid(lm_utl_load)

ggplot(temp, aes(x = resid_utlload, y = resid_costload)) +
  geom_point() +
  geom_smooth(method = "lm", se = FALSE) +
  geom_smooth(method = "loess", se = FALSE) +
  geom_hline(yintercept = 0, linetype = "dashed") +
  labs(title = "Partial Residual Plot - Add utl",
       x = "Residuals cost ~ load", y = "Residuals utl ~ load")


lm_cost_utl <- lm(cost ~ utl, data = airdata)
temp <- airdata
temp$resid_costutl <- resid(lm_cost_utl)
lm_load_utl <- lm(load ~ utl, data = temp)
temp$resid_loadutl <- resid(lm_load_utl)

ggplot(temp, aes(x = resid_loadutl, y = resid_costutl)) +
  geom_point() +
  geom_smooth(method = "lm", se = FALSE) +
  geom_smooth(method = "loess", se = FALSE) +
  geom_hline(yintercept = 0, linetype = "dashed") +
  labs(title = "Partial Residual Plot - Add load",
       x = "Residuals cost ~ utl", y = "Residuals load ~ utl")
```
