...
To get this from a theoretical framework into empirical questions, we focus on two main research questions.

1. **Univariate analysis**: What is the association between poverty rates (*PctPopUnderPov*) and violent crime rates (*ViolentCrimesPerPop*) in U.S. communities? This analysis gives us an initial understanding of whether our hypothesis holds, how strong the relationship is, without taking confounding factors into account.

2. **Multivariable analysis**: How does the association between poverty and violent crime rates change when we do account for other socio-economic and demographic factors? Specifically, we examine whether the poverty-crime relationship is confounded by or interacts with variables defined before. This analysis allows us to better isolate the independent effect of poverty.

The univariate model will establish whether there is evidence for Becker's economic theory in the raw data, while the multivariable model provides a better understanding of how poverty operates within a complex socio-economic context. The latter is particularly important for policy recommendations, as it reveals whether poverty reduction alone would reduce crime rates, or whether interventions must also address educational disparities, employment opportunities, or other contextual factors.
...
Socioeconomic disadvantage can be estimated from multiple variables in the study. Due to the large number of options and nature of our research question, it was decided that finding an optimal model using a limited number of predictors is beneficial to answer the question in mind. For this, we intend to use an All-possible regressions procedure on the 80% of the data labeled for training in order to select the most meaningful variables. We only include models with a maximum of 5 predictor variables, and only models with 0, 1 or 2 education predictor variables will be evaluated. In order to decide the best model, we intend to choose the best model under BIC. This measure penalizes for adding more variables to the model in a large sample. To test if the chosen variables are in the right format in the model, we can use partial regression plots to visualize the effect of the specific variables on the rest of the model. If needed transformations may be used. To choose which interaction terms to include, we add all interaction terms of the predictor variables with our main predictor variable to the selected multivariate model and evaluate their significance. We retain the interaction term(s) that most improve the model, including at least one.  
...
# olsrr::ols_plot_dfbetas(fit_final)

olsrr::ols_plot_diagnostics(fit_final)

# car::influenceIndexPlot(fit_final)

#car::dfbeta(s)Plots(fit_final): (un)standardized
...
Another caveat is that the FBI’s data is from 1995, while the Census populations containing demographic variables are from 1990. This induces measurement error in the dependent variable. As the violent crime rate is calculated using 1995 population estimates in the denominator, the standard error predictors are from 1990. Thus, communities that experience large population growth or decline between 1990 and 1995 have biased crime rates. Growing communities have understated rates, declining communities have overstated rates.  Also, communities with larger population differences will have more noisy crime rate measurements (leading to heterosked?). This leads to measurement error in the dependent variable, leading to heterosked and outliers. (not entirely sure about this paragraph).

At last, the data are from 1995. Since then, the social and economic environment have changed, implying that our estimated relationships would not hold today as crime rates, reporting practices,... have evolved. Thus, these results should be carefully intepreted in today's context as the mechanisms driving these relationships are different in comparison to when the data collection happened.  
