...
The effect of PctPopUnderPov varies with PctBsorMore through the interaction terms. This means that the effect of PctPopUnderPov is not simply given by its coefficient, but instead we can look at the estimated effect of PctPopUnderPov on the expected value of the logarithm of (violent crime rate per population of 100k + 1) as `r b["PctPopUnderPov"]` + `r b["PctPopUnderPov:PctBSorMore]` * PctBSorMore. If we take for example the mean of the PctBSorMore to calculate the effect of PctPoponderpov when the interacting variable is held at its average and all variables are held constant, we can find `r coef_mean`. The confidence interval of this effect is [`r ci_lower`, `r ci_higher`]
...
For the percentage of people between the age of 12 and 29, we find a negative association with the logarithm of (violent crime rate per population of 100k + 1). This is expressed by a coefficient of `r b[agePct12t29]` with 95% confidence interval [`r confidence_intervals[5,1]`, `r confidence_intervals[5,2]`]. With 95% confidence we estimate that the true coefficient of agePct12t29  is somewhere between `r confidence_intervals[5,1]` and `r confidence_intervals[5,2]` when all other variables are fixed. Increasing percentage of agePct12t29 is associated with a decrease in the mean logarithm of (violent crime rate per population of 100k + 1). This negative association is not in line with our prior assumption that violent crimes would increase for this age group.
...
validation_logscale <- c(round(mspr_test_log, 2), round(mse_train_log, 2), round(mspr_test_log/mse_train_log, 4), round(r2_test_log, 4), round(r2_train_log, 4))
```
...
# MSE for training data
mse_train <- sum((train_data$ViolentCrimesPerPop - pred_interval_train[, "fit"])^2)/(n_train - p)

cat("MSPR:", round(mspr_test, 2), "\n")
cat("MSE:", round(mse_train, 2), "\n")
cat("Ratio MSPR/MSE:", round(mspr_test/mse_train, 4), "\n")
...
# Results for multivariate model backtransformed
validation_backtrans <- c(round(mspr_test, 2), round(mse_train, 2), round(mspr_test/mse_train, 4), round(r2_test, 4), round(r2_train, 4))
```
...
validation_simple <- c(round(mspr_test_simple, 2), round(mse_simple, 2), round(mspr_test_simple/mse_simple, 4), round(r2_test_simple, 4), round(r2_train_simple, 4))

df_validation <- data.frame(multivar_backtransformed = validation_backtrans, multivar_logscale = validation_logscale, univar = validation_simple)
rownames(df_validation) <- c("MSPR", "MSE", "Ratio MSPR/MSE", "R² (test)", "R² (training)")
df_validation
```
...
mse_final <- sum(fit_final$residuals^2)/(n_training - p)
