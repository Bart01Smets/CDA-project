...
temp <- train_data
lm_ViolentCrimesPerPop <- lm(ViolentCrimesPerPop ~ PctLess9thGrade + PctNotHSGrad + racepctblack + PctImmig, data = temp)
temp$resid_outcome <- resid(lm_ViolentCrimesPerPop)
lm_PctPopUnderPov <- lm(PctPopUnderPov ~ PctLess9thGrade + PctNotHSGrad + racepctblack + PctImmig, data = temp)
temp$resid_pred <- resid(lm_PctPopUnderPov)
...
ggplot(temp, aes(x = resid_pred, y = resid_outcome)) +
  ...
  labs(title = "Partial Residual Plot - Add PctPopUnderPov",
       x = "Residuals PctPopUnderPov ~ other variables", y = "Residuals ViolentCrimesPerPop ~ other variables")
```
