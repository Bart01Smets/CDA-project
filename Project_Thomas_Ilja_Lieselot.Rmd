...
y_median <- median(crimes_table_subset$ViolentCrimesPerPop)

crimes_table_subset$y[crimes_table_subset$y <= y_median] <- 0
crimes_table_subset$y[crimes_table_subset$y > y_median] <- 1
```
...
For the second part of this project, the response variable *ViolentCrimesPerPop* was dichotomized to fit a generalized linear model. Therefore the whole dataset was split at the median value to create 2 categories, the lower half of the response variables coded as 0 (low violent crime rate) and the top half of the response variable coded as 1 (high violent crime rate). 
...
All $\beta$ coefficients are significant according to their Wald test statistics, except for the $\beta$ coefficient for the interaction effect *PctPopUnderPov:PctBSorMore*. Because of this reason it is decided that the interaction effect should be eliminated from the model. After fitting the model again, all $\beta$ coefficients are significant.
...
outliers <- which(abs(res) > 3) # arbitraraly chosen to get top 2 residuals, the outliers
train_data[outliers,]
predictions_outliers <-  predict(glm_fit, newdata=train_data[outliers,], type = "response")
```
A half-normal plot is used to visualize the absolute values of the standardized deviance residuals plotted against the theoretical quantiles. This visualisation allows us to access the goodness of fit, potential overdispersion, and visualize potential outliers. For our model, we can see that most of the points fall approximately within the simulated envelope around the theoretical line, showing that the model fits the data reasonably well. However, it could be seen that one point, the community of Douglascity, was seen outside this boundary showing that this points have larger-than-expected residuals. These points can be seen as outliers to our model. This points had a high value in the *PctPopUnderPov* and *PctImmig* even reaching among the more uncommon values. Given that our model associates these variables with a high crime rate the higher the value, the model would suspect these to get a 1 value, however this data point was among the lower half of the response variables giving it a 0 value in the binary model. Predicting this points with our model confirms this as the model predicts a value of `r round(predictions_outliers[1], 3)`, clearly indicating that the model will missclassify this point.

### Prediction and model vaildation
...
glm_fit_full <- glm(y ~ PctPopUnderPov + PctBSorMore + agePct12t29 + PctImmig + race_black_logit, data = crimes_table_subset, family = binomial)
...
for (variable in attr(terms(glm_fit), "term.labels")){
  print(plot(effect(variable,glm_fit)))
} 
```
Based on the effect plot of *PctPopUnderPov*, it could be seen that a higher percentage of the population for a state living in poverty also results in a higher probability that the violent crimes are among the higher values in the United states. It could also be seen that the 95% Wald confidence band for the predicted probabilities widens when the *PctPopUnderPov* gets larger, probably due to the sparsity of the data at these extreme percentages. Similarly for the other predictors, a slope could be seen in the effect plots in the same direction as the $\beta$ for the predictor. Each time, it could be observed that the in the edges, where data becomes more sparse, the confidence band starts to widen.

### Conclusion

The logistic model produces predictions with an error rate of `r round(error_rate_train, 3)` on training data and `r round(error_rate_test, 3)` on test data, and an AUC of `r round(calc_auc(roc)["AUC"], 3)`. With the exception of the interaction term *PctPopUnderPov:PctBSorMore*, all predictors are statistically significant. All coefficients have the same sign as in the linear model. Both models therefore lean towards the same results, though it's hard to compare the two as the scales used in the models are not comparable. Where more subtle results are required, for example to inform policies to lower crime rates, the linear model could be preferred as the binary outcome in the logistic model doesn't allow to differentiate between communities within the higher crime rates (binary outcome = 1). It is important to note that the same limitations of the data and demographic nature of the variables mentioned for the multivariate regression apply here and no causal relationships are identified. 
...
# Figures Catergorical part:

##### Residual plots:

```{r ref.label="residuals_bin", include=TRUE, fig.width=8, fig.height=6}

```

##### Half normal plot:

```{r ref.label="half_normalplot", include=TRUE, fig.width=8, fig.height=6}

```

##### Prediction plots:

```{r ref.label="model_plots_glm", include=TRUE, fig.width=8, fig.height=6}

```

##### Roc curve:

```{r ref.label="theRockIsBald", include=TRUE, fig.width=8, fig.height=6}

```

##### Effect plots:

```{r ref.label="effect_plots", include=TRUE, fig.width=8, fig.height=6}
