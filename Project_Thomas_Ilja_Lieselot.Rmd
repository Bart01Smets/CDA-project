...
All $\beta$ coefficients are significant according to their Wald test statistics, except for the $\beta$ coefficient of the interaction effect *PctPopUnderPov:PctBSorMore*. Because of this reason it is decided that the interaction effect should be eliminated from the model. After fitting the model again, all $\beta$ coefficients are significant.
...
```
A half-normal plot is used to visualize the absolute values of the standardized deviance residuals plotted against the theoretical quantiles. This visualisation allows us to assess the goodness of fit, potential overdispersion, and visualize potential outliers. For our model, we can see that most of the points fall approximately within the simulated envelope around the theoretical line, showing that the model fits the data reasonably well. However, it could be seen that one point, the community of Douglascity, was seen outside this boundary showing that this point has larger-than-expected residuals and can be seen as an outlier to our model. This point had a high value in the *PctPopUnderPov* and *PctImmig* even reaching among the more uncommon values. Given that our model associates these variables with a high crime rate, the higher the value, the more the model would suspect these to get a 1 value, however this data point was among the lower half of the response variables giving it a 0 value in the binary model. Predicting this point with our model confirms this as the model predicts a value of `r round(predictions_outliers[1], 3)`, clearly indicating that the model will misclassify this point.
