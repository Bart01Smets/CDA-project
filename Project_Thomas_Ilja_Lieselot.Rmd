...
#train_data[["ViolentCrimesPerPop"]] == 0

# Residuals berekenen
raw_res <- residuals(glm_fit, type = "response")                 # Raw residuals
stud_pearson_res <- rstandard(glm_fit, type = "pearson")        # Pearson residuals
stud_deviance_res <- rstandard(glm_fit, type = "deviance")      # Deviance residuals
stud_student_res <- rstudent(glm_fit)                            # Studentized residuals

# Estimated probabilities
estimated_prob <- fitted(glm_fit)    


ggplot(plot_df, aes(x = prob, y = stud_pearson_res)) +
  geom_point(alpha = 0.1) +
  geom_smooth(se = FALSE, method = "loess") +
  scale_x_continuous(breaks = seq(0,1,0.1)) +
  scale_y_continuous(limits = c(-3,3)) +
  labs(x = "estimated probability", y = "studentized Pearson residuals")
...
### Cooks
dit is gewoon die van thomas overgenomen, geen idee of dat mag op deze manier voor binary

```{r}
library(olsrr)
olsrr::ols_plot_cooksd_bar(glm_fit)
```
