...
```
An half-normal plot was used to visualize the absolute values of the standardized deviance residuals plotted against the theoretical quantiles. This visualisation allows us to acces the goodness of fit, potential overdispersion, and visualize potential outliers. For out model, we could see that most of the points fall approximatly within the simulated envelope around the theoretical line, showing that the model fits the data reasonably well. However, it could be seen that one point was seen outside this boundery and one on the boundry itself, showing that these points have larger-than-expected residuals. These points could be seen as outliers to our model. Both these points had a high value in the *PctPopUnderPov* and *PctImmig* even reacing among the more uncommon values. Given that our model associates these varibales with a high crime rate the higher the value, the mmodel would suspect these to get a 1 value, however these datapoints themselves are not among the higher portion of the response variable. Predicting these points with our model confirms this as the model predicts a value of `r round(predictions_outliers[1], 3)` and `r round(predictions_outliers[2], 3)` for these 2 points
