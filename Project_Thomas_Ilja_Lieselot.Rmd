...
All $\beta$ coefficients are significant according to their Wald test statistics, except for the $\beta$ coefficient for the interaction effect *PctPopUnderPov:PctBSorMore*. Because of this reason it's decided to eliminate the interaction effect from the model.
...
After fitting the model again, all $\beta$ coefficients are significant.

The estimated value for $\beta_{\text{PctPopUnderPov}}$ = `r round(glm_fit$coefficients["PctPopUnderPov"], 5)` suggests that more poverty is associated with a higher chance for a large number of violent crimes per 100k population. In fact, the odds of success increases with a factor of exp(`r round(glm_fit$coefficients["PctPopUnderPov"], 5)`) = `r round(exp(glm_fit$coefficients["PctPopUnderPov"]), 5)` for each 1 percentage increase in percentage poverty.
