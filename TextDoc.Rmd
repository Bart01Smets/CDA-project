...
For the percentage of people between the age of 12 and 29, we find a negative association with the logarithm of (violent crime rate per population of 100k + 1). This is expressed by a coefficient of `r round(b["agePct12t29"], 4)` with 95% confidence interval [`r round(confidence_intervals[5,1], 5)`, `r round(confidence_intervals[5,2], 5)`]. With 95% confidence we estimate that the true coefficient of *agePct12t29* is  between `r round(confidence_intervals[5,1], 5)` and `r round(confidence_intervals[5,2], 5)` when all other variables are fixed. Increasing percentage of *agePct12t29* is associated with a decrease in the mean logarithm of (violent crime rate per population of 100k + 1). This negative association is not in line with our prior assumption that violent crimes would increase for this age group.
...
```{r ref.label="simple-lm-diagnostics"}
...
how can i reference to figures in my appendix in r since this is after all text
