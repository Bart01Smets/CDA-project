...
By examining the missing data, there were 221 (of the 1994) observations identified as NA values for the outcome variable *ViolentCrimesPerPop*. No imputations were used to have a more transparent model building procedure and to avoid overestimating the quality of the model fit. As already mentioned in the section on the study design, these NA values are possibly due to the fact that rape counts, a component of violent crime, were not included in the statistics for several states. The observations where the outcome variable has an NA value are removed, as these rows are not useful for the regression. It can be noted that the variables *countyCode* and *communityCode* are also frequently unknown.
...
After investigating the univariate descriptives, multivariate descriptives are looked at.
...
```
To gain insight into how the different variables in the dataset are correlated with each other, the correlation coefficients are computed and scatter plots are investigated showing the relationship between the variables and the outcome variable (*ViolentCrimesPerPop*) and the main predictor variable (*PctPopUnderPov*).

The following predictors are highly correlated with each other.

-   *PctNotHSGrad* and *PctLess9thGrade* ($r = 0.93$)
-   *perCapInc* and *PctBSorMore* ($r = 0.77$)
-   *PctNotHSGrad* and *PctBSorMore* ($r = -0.75$)

It is noticeable that the variable *racepctblack* is the one most strongly correlated with the outcome variable ($r = 0.63$), even more than *PctPopUnderPov*, the main predictor that was chosen for this research.

The scatter plots showed that most variables display a roughly linear relationship with *ViolentCrimesPerPop*, although that trend is often distorted in the extreme regions of the x-axis. *agePct12t29* has a very low correlation coefficient with *ViolentCrimesPerPop* (0.11). *perCapInc* shower a higher correlation (-0.32), but there is clearly no linear trend present.

Some variables exhibit a linear relationship with *PctPopUnderPov* or have a high correlation with each other. This suggests that evaluating multicollinearity during the model-building stage is recommended.
...
Using the holdout set, we compute the Mean Squared Prediction Error (MSPR) and comparing it to the MSE.
...
The $R^2$ of the multivariate model is higher than the $R^2$ of the univariate model. However, these values are difficult to compare because the outcome variable differs between the two models ($ViolentCrimesPerPop$ vs. $\log(ViolentCrimesPerPop)$). Since the logarithmic transformation is non-linear, back-transforming the predicted values of $\log(ViolentCrimesPerPop)$ does not yield the expected value ($E(\log(Y) \mid X = x) = \beta_0 + \beta_1 x ;\nRightarrow; E(Y \mid X = x) = e^{\beta_0 + \beta_1 x}$), except under strict assumptions regarding the residuals.
