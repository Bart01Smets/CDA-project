...
It was also investigated whether or not small communities have a higher probability to have more extreme values of the response and predictor variables. Scatter plots show that communities with a very small population indeed show a very large spread for all variables (fig. \@ref(fig:scattersforpop)).
...
For the simple linear model the assumptions of linearity are violated. Larger outcome values tend to be underestimated, the variance for larger outcome values is larger and the QQ-plot shows violation of the normality assumption. Regarding the population size, it can be noted that the residuals for smaller populations are rather negative, while the residuals for larger populations are rather positive. This means that the model tends to underestimate the total number of crimes for large populations and overestimate, while the total number of crimes for small populations are rather underestimated.
...
$$
\begin{aligned}
ViolentCrimesPerPop_i =\;& 7.358 
    + 0.026 \cdot PctPopUnderPov_i - 0.026 \cdot PctBSorMore_i - 0.026 \cdot agePct12t29_i + 0.030 \cdot PctImmig_i\\
& + 0.273 \cdot race\_black\_logit_i + 0.0008 \cdot PctPopUnderPov_i \cdot PctBSorMore_i + \epsilon_i\\
\end{aligned}
$$
...
=======
$$
ViolentCrimesPerPop_i = 7.358 + 0.026 \cdot PctPopUnderPov_i - 0.026 \cdot PctBSorMore - 0.026 \cdot agePct12t29  + 0.030 \cdot PctImmig_i + 0.273 \cdot race_black_logit + 0.0008 \cdot PctPopUnderPov:PctBSorMore + \epsilon_i
$$
```{r vivs2}
>>>>>>> 3abb57d (Part 2 figures)
vif <- vif(fit_final)
print(vif)
cat("BIC: ", BIC(fit_final))
```
...
```{r correlatiion_plot_discriptives, ref.label="cor_discriptives", include=TRUE}

```

scatter plots:

```{r scatters_descriptives, ref.label="scatter plots", fig.width=8, fig.height=10, include=TRUE}

```

population scatter plots:

```{r scattersforpop, ref.label="scatter plots population", fig.width=8, fig.height=10, include=TRUE}
...
```{r ref.label="simple-lm-diagnostics", include=TRUE}

```

Residuals based on population size:

```{r ref.label="outliers_vs_population1", include=TRUE}
...
```{r ref.label="multimodel1_model_diagnostics", include=TRUE}

```

Assumption checks log transformation test: 
```{r ref.label="logtransformed_first_model", include=TRUE}

```
Partial regression plots first log model:

```{r ref.label="partial-regression-plots2", include=TRUE}

```

Logit transformed *racepctblack* model assumsions check:

```{r ref.label="logit_model_test"}

```

Partial regression logits:

```{r ref.label="partial-regression-logit"}

```

VIF check model with *less9thgrade*
```{r ref.label="VIFs_model"}

```

Assumptions check one education model:

```{r ref.label="model_best1education"}
...
```{r ref.label="partials_final_model"}

```

Plot of outliers vs population:

```{r ref.label="outliers_vs_population2"}
